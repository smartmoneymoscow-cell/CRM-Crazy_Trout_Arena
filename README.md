...
- 🎯 **Фильтры** — dropdown через Stack (не Overlay), нижнее меню перекрывает список при пересечении
...
### Отчёт и финансы
- 📊 **Финансовый дашборд** — выручка со спарклайном, маржинальная прибыль, переменные расходы
- 📈 **KPI-карточки** — средний чек, LT/LTV (только число + посещения), средний улов (штуки/кг), возвращающиеся клиенты, рейтинг ★ (звезды справа от числа)
- 🥧 **Структура выручки** — кольцевая диаграмма (вторая на странице), рубль в центре на одной строке с числом
- 🐟 **Декомпозиция продаж** — круговая диаграмма по 5 породам + тарифы (в шт.)
- 📉 **Динамика выручки** — квартал по умолчанию, обновление при смене периода/календаря
