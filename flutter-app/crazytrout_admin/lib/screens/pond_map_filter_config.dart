/// Константы и логика фильтров карты пруда.
/// Вынесены из pond_map_screen.dart для unit-тестирования.
/// ─────────────────────────────────────────────────────────
/// ⚠️ НЕ ИЗМЕНЯТЬ без обновления тестов в test/screens/pond_map_filter_test.dart.
///
/// Требования к dropdown фильтров:
///
///   1. Dropdown НИКОГДА не перекрывает нижнее меню
///   2. При пересечении с нижним меню — скрывается СТРОГО ПОД меню
///   3. ОБЯЗАТЕЛЬНЫЙ ТЕСТ: dropdown скрывается под нижним меню
///   4. Прикреплён к нижнему краю карточки, без зазора (gap = 0)
///   5. Углы: кнопка — нижние выпрямляются при раскрытии, верхние НЕИЗМЕННЫ (pill 999).
///      dropdown — верхние квадратные (0), нижние скруглены (12).
///   6. Сворачивается: выбор опции, повторный тап, тап в пустую область
///   7. НИКОГДА не сворачивается при скролле — скроллится со страницей
///   8. НИКОГДА не блокирует скролл страницы
///   9. ВСЕГДА полная высота, НИКОГДА не меняет высоту
///  10. НИКОГДА не открывается вверх — только вниз. ОБЯЗАТЕЛЬНЫЙ ТЕСТ
///  11. НИКОГДА не сдвигает контент. ОБЯЗАТЕЛЬНЫЙ ТЕСТ
///  12. z-order: над контентом, ПОД нижним меню
///  13. Верхние углы кнопки НИКОГДА не меняются при раскрытии — всегда pill (999).
///      Нижние углы выпрямляются (0) при раскрытии dropdown.
///      ОБЯЗАТЕЛЬНЫЙ ТЕСТ
///  14. Dropdown в Stack внутри body (НЕ в Overlay) — z-order под нижним меню.
///      ОБЯЗАТЕЛЬНЫЙ ТЕСТ
///  15. Dropdown той же ширины что кнопка, прикреплён к низу кнопки без зазора
///      и без смещения вправо/влево. ОБЯЗАТЕЛЬНЫЙ ТЕСТ
///
/// ┌─────────────────────────────────────────────────────────────────┐
/// │ ТАБЛИЦА КРИТИЧЕСКИХ БАГОВ (НЕ ПОВТОРЯТЬ)                       │
/// ├─────────────────────────────────────────────────────────────────┤
/// │ # │ Баг                          │ Почему критический           │
/// ├───┼──────────────────────────────┼──────────────────────────────┤
/// │ 1 │ Контент двигается вниз       │ inline Stack двигает контент │
/// │ 2 │ Dropdown сжимается           │ пункты нечитаемые            │
/// │ 3 │ Dropdown закрывается         │ теряется контекст фильтрации │
/// │ 4 │ Dropdown летает при скролле  │ визуальный глитч             │
/// │ 5 │ Залезает за нижнее меню      │ нарушает z-order             │
/// │ 6 │ Скроллится внутрь            │ два скролла одновременно     │
/// │ 7 │ Зазор между кнопкой и списком│ визуальный разрыв            │
/// │ 8 │ Нижние углы кнопки круглые   │ визуальный баг при раскрытии │
/// └─────────────────────────────────────────────────────────────────┘

enum FilterValue { none, all, premium, standard, basic }

const Map<FilterValue, String> filterOptions = {
  FilterValue.none: 'Нет',
  FilterValue.all: 'Все клиенты',
  FilterValue.premium: 'Премиум',
  FilterValue.standard: 'Стандарт',
  FilterValue.basic: 'Базовый',
};

const Map<FilterValue, String> filterButtonLabels = {
  FilterValue.none: 'Фильтры',
  FilterValue.all: 'Все',
  FilterValue.premium: 'Премиум',
  FilterValue.standard: 'Стандарт',
  FilterValue.basic: 'Базовый',
};

/// Ширина кнопки фильтров и dropdown (должны совпадать).
const double kDropdownWidth = 120.0;

/// Зазор между кнопкой и дропдауном. Должен быть 0 (вплотную).
const double kDropdownGap = 0.0;

/// Высота одного пункта меню дропдауна.
const double kDropdownItemHeight = 44.0;

/// Вертикальный padding внутри дропдауна.
const double kDropdownVPadding = 8.0;

/// Высота нижней навигации (BottomNavigationBar + SafeArea).
const double kBottomNavHeight = 60.0;

/// Высота строки фильтров (кнопка + padding).
const double kFilterRowHeight = 36.0;

/// Радиус «пилюли» для кнопки фильтров.
/// Должен быть заведомо МЕНЬШЕ реальной высоты кнопки (~32-34dp),
/// чтобы Flutter НЕ клэмпил радиус и верхние углы оставались
/// одинаковыми независимо от нижних (закрыто/открыто).
/// См. подробности в build() FiltersDropdown.
const double kPillRadius = 18.0;
